/**
 * Prevent the missing console object on old browsers
 *
 * @return object
 */
(function(){
...
/**
 * Detect if the user devise is a mobile or not
 * 
 * @return bool
 */
(function(){
...
(function(){
	...
	var browser = function(){
		this.name = searchString.call(this,dataBrowser) || "an unknown browser";
		this.version = searchVersion.call(this,navigator.userAgent)
			|| searchVersion.call(this,navigator.appVersion)
			|| "an unknown version";
		this.OS = searchString.call(this,dataOS) || "an unknown OS";
		this.isIE = (this.name.s == "MSIE")? this.version : navigator.userAgent.indexOf("Trident") != -1? true : false;
		instance = this;
	};
		...
		for (var i=0;i<data.length;i++)	{
			...
			if (dataString) {
				if (dataString.indexOf(data[i].subString) != -1){
					return {l:data[i].identity,s:(data[i].subString !== undefined && data[i].subString !== data[i].identity)? data[i].subString : ""};
				}
			}
			else if (dataProp)
	...
	if(instance == null) window.browser = new browser();
	else window.browser = instance;
})();